...
struct residualByIdx : public thrust::binary_function<Tin, Tin, Tout>
{
    const Tout *d_residual_raw;
    const Tout *d_mod_delta_raw;

    __host__ __device__
    residualByIdx(const Tout *_res, const Tout* _md) : d_residual_raw(_res), d_mod_delta_raw(_md) {}

    __host__ __device__ Tout operator() (Tin idx, Tin midx)
    {
        return d_residual_raw[idx] + d_mod_delta_raw[midx];
    }
};
    ...
    for (uint32_t i = 0; i < ions.size(); i++) {
        ...
        if (isMa) {
            ...
            for (uint32_t k = 0; k < ma_idx; k++) {
                unrank_idx += h_ma_count[k];                
            }
            bool modded = false;
            // check through unrank to see if the ith_ma is to be modded
            for (uint32_t l = 0; l < h_ma_count[ma_idx]; l++) {
...
getSpecNonParallel(
    ...
    const uint32_t      *d_mpep_pep_idx_raw,
...
)
{
    ...
    thrust::device_ptr<const uint32_t>      d_mpep_pep_idx(d_mpep_pep_idx_raw);
    thrust::device_ptr<const uint32_t>      d_mpep_pep_mod_idx(d_mpep_pep_mod_idx_raw);
    thrust::device_ptr<const uint32_t>      d_mpep_unrank(d_mpep_unrank_raw);
    thrust::device_ptr<const uint32_t>      d_mpep_mod_ma_count_sum_scan(d_mpep_mod_ma_count_sum_scan_raw);
    ...
    thrust::device_ptr<const uint8_t>       d_ma(d_ma_raw);
    thrust::device_ptr<const float>         d_ma_mass(d_ma_mass_raw);
    ...
    std::vector<uint8_t> h_ma(num_ma);
    thrust::copy(d_ma, d_ma + num_ma, h_ma.begin());

    // foreach unrank
    for (uint32_t i = 0; i < num_mpep; ++i) {
        // add mions and annotate tc tn
        uint32_t idx = d_mpep_pep_idx[i];
        ...
        std::vector<uint32_t> h_ma_count(num_ma);
        thrust::copy(d_mod_ma_count + mod_idx*num_ma, d_mod_ma_count + (mod_idx+1)*num_ma, h_ma_count.begin());
...
//#ifdef _DEBUG
        ...
        //std::cout << " | ";
//#endif
        std::vector<uint8_t> mions = genMIons(ions, unrank, h_ma, h_ma_count);
...
//#endif
    ...
    thrust::transform(d_mpep_pep_idx, d_mpep_pep_idx + num_mpep, d_mpep_pep_mod_idx, d_check_residual.begin(), residualByIdx<uint32_t, float>(d_residual_raw, d_mod_delta_raw));
    // d_check_mass      mass of amino acid
    ...
    for (uint32_t i = 0; i < num_ma; ++i) {
        d_check_mass[toLower(d_ma[i]) - 'A'] = d_mass[d_ma[i] - 'A'] + d_ma_mass[i];
    }
    // d_check_ions      amino acid chars. modified acid mass is lower case
    ...
    // print some things
    std::cout << "num_ma " << num_ma << std::endl;
    ...
    for (uint32_t i = 0; i < num_mpep; ++i) {
        uint32_t mod_idx = d_mpep_pep_mod_idx[i];
        float p_mass = (d_residual[d_mpep_pep_idx[i]]+ d_mod_delta[mod_idx]);
        ...
        if (d_check_residual[i] != p_mass ){
            std::cerr << "orig " << d_residual_raw[d_mpep_pep_idx[i]] << std::endl;
