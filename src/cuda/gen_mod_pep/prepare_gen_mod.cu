...
struct prepare : public thrust::unary_function<T, void>
{
    ...
    prepare(uint32_t*       _ompi,
            ...
            d_out_mpep_pep_idx(_ompi), d_out_mpep_rank(_omr), d_out_mpep_ith_valid(_omiv), d_out_mpep_mod_ma_count_sum(_ommmcs), d_mod_ma_count_sum(_mmcs), d_pep_idx(_pi), d_pep_mod_idx(_pmi), d_pep_valid_idx(_pvi), d_pep_num_mpep(_pnm), d_pep_num_mpep_scan(_pnms) {}
    ...
    __host__ __device__ void operator() (T ith_valid)
    {
        ...
        const uint32_t num_pep = d_pep_num_mpep[ith_valid];
...
)
{
    ...
    thrust::exclusive_scan(d_out_mpep_mod_ma_count_sum_th, d_out_mpep_mod_ma_count_sum_th + num_pep, d_out_mpep_mod_ma_count_sum_scan_th);
